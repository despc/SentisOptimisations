namespace SpaceEquipmentLtd.NanobotBuildAndRepairSystem
{
   ...
   */
   ...
   public class NanobotBuildAndRepairSystemMod : MySessionComponentBase
   {
      ...
      private static TimeSpan SourcesAndTargetsUpdateTimerInterval = new TimeSpan(0, 0, 10);
      ...
      public const int MaxBackgroundTasks_Default = 6;
         ...
         lock (AsynActions)
         {
            ...
            if (ActualBackgroundTaskCount < Settings.MaxBackgroundTasks)
            {
               ...
               MyAPIGateway.Parallel.StartBackground(() =>
               {
                  try
                  {
                     int maxAsyncTaskInBatch = 2;
                     int currentBatchAsyncTasks = 0;
                     while (currentBatchAsyncTasks < maxAsyncTaskInBatch)
                     {
