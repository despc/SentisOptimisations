...
namespace SpaceEquipmentLtd.NanobotBuildAndRepairSystem
{
   ...
   public class NanobotBuildAndRepairSystemBlock : MyGameLogicComponent
   {
      ...
      private static readonly int MaxTransportEffects = 1;
      private static int _ActiveTransportEffects = 0;
      private static readonly int MaxWorkingEffects = 1;
      ...
      private bool ServerDoGrind(TargetBlockData targetData, out bool transporting)
      {
         ...
         if (_Welder.OwnerId == 0)
         {
            Tools.Log(MyLogSeverity.Error, "Try grind with nobody nano " + _Welder.SlimBlock.BuiltBy);
            return false;
         }
         if (!cubeGrid.DestructibleBlocks)
         {
            Tools.Log(MyLogSeverity.Error, "Try grind protected grid " + _Welder.SlimBlock.BuiltBy);
            return false;
         }
         if (AdminProtectorBlock.ImmortalGrids.Contains(targetGrid.EntityId))
         {
            return false;
         }
         ...
         var grindingMultiplier = _Welder.CubeGrid.IsStatic ? NanobotBuildAndRepairSystemMod.Settings.Welder.GrindingMultiplier : 1;
         float damage = MyAPIGateway.Session.GrinderSpeedMultiplier * grindingMultiplier * GRINDER_AMOUNT_PER_SECOND;
                     ...
                     try
                     {
                        ...
                        for (var idx = 0; idx < maxInv; idx++)
                        {
                           ...
                           if (!possibleSources.Contains(inventory) 
                               //&& inventory.IsConnectedTo(welderInventory)
                               )
                           {
